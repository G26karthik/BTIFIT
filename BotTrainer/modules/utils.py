...
def sanitize_input(text: str, max_len: int = 1000) -> str:
    """Strip control characters and enforce a max length on user input."""
    return re.sub(r'[\x00-\x08\x0b\x0c\x0e-\x1f]', '', text[:max_len]).strip()
...
def setup_logging(level: int = logging.INFO, log_file: str = "bottrainer.log") -> None:
...
if __name__ == "__main__":
    setup_logging()
    print("═" * 50)
    print("  Module 1: utils.py — Utility Functions Demo")
    print("═" * 50)
    ...
    # 2. JSON parsing (clean)
    sample_json = '{"intent": "book_flight", "confidence": 0.95, "reasoning": "User wants to book"}'
    parsed = safe_json_loads(sample_json)
    print(f"✅ safe_json_loads() → {parsed}")

    # 3. JSON recovery (truncated — simulating broken LLM output)
    broken_json = '{"intent": "balance", "confidence": 0.88'
    recovered = safe_json_loads(broken_json)
    print(f"✅ safe_json_loads() (truncated recovery) → {recovered}")

    # 4. Validate intent result
    known = {"book_flight", "balance", "transfer", "out_of_scope"}
    validated = validate_intent_result(parsed, known)
    print(f"✅ validate_intent_result() → {validated}")

    # 5. Truncate text
    long_text = "A" * 300
    short = truncate_text(long_text, max_length=50)
    print(f"✅ truncate_text() → '{short}' (len={len(short)})")

    # 6. File I/O
    from pathlib import Path
    import tempfile, os
    tmp = Path(tempfile.mkdtemp()) / "test.json"
    save_json_file({"test": True}, tmp)
    loaded = load_json_file(tmp)
    print(f"✅ save_json_file() + load_json_file() → {loaded}")
    os.remove(tmp)
