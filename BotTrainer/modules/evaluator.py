...
from BotTrainer.modules.utils import load_json_file, save_json_file
...
class Evaluator:
    ...
    def run_evaluation(
        ...
        for i, text in enumerate(texts):
            raw_results.append(classifier.classify(text))
...
if __name__ == "__main__":
    ...
    print("═" * 50)
    print("  Module 5: evaluator.py — Evaluation Demo")
    print("═" * 50)

    _PROJECT_ROOT = Path(__file__).resolve().parent.parent
    _INTENTS_PATH = _PROJECT_ROOT / "data" / "intents.json"
    _EVAL_PATH = _PROJECT_ROOT / "data" / "eval_dataset.json"

    if not _INTENTS_PATH.exists() or not _EVAL_PATH.exists():
        ...
        try:
            ...
            # Run a small evaluation (10 samples)
            print("\n⏳ Running evaluation on 10 samples ...\n")
            results = evaluator.run_evaluation(
                eval_dataset_path=_EVAL_PATH,
            ...
            print(f"\n   Errors: {len(errors)} / {len(results['y_true'])} misclassified")
